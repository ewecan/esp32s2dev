...
void displayMenuSelect_BRT(void)
{
	...
	TFT_Display_Image_Select(CS4_PIN, pic_systemTheme_SetBRT);
	TFT_Display_Image_Select(CS3_PIN, pic_systemTheme_Brt);
}
/**
 * @brief 欢迎界面 按键操作
 * @param  key  按键键码
...
void displayMenuSelect_BRT_KEY(int8_t key)
{
	...
	case 2: //按键2  下
	{
		...
		TFT_Display_Image_Select(CS2_PIN, pic_systemTheme_Brt);
		TFT_Display_Num(CS3_PIN, TFTConfig.BrtLevel);
		TFT_Display_Image_Select(CS4_PIN, pic_systemTheme_zero);
		TFT_Display_Image_Select(CS5_PIN, pic_systemTheme_Percent);
