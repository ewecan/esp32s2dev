...
void displayMenuSelect_PIC(void)
{
	...
	TFT_Display_Image_Select(CS4_PIN, pic_systemTheme_SetPIC);
	TFT_Display_Image_Select(CS3_PIN, pic_systemTheme_Pic);
}
/**
 * @brief 欢迎界面 按键操作
 * @param  key  按键键码
...
void displayMenuSelect_PIC_KEY(int8_t key)
{
	...
	case 2: //按键2  下
	{
		...
		TFT_Display_CLK_Num(CS3_PIN, 9, TFTConfig.theme_index);
		TFT_Display_CLK_Num(CS4_PIN, 1, TFTConfig.theme_index);
		TFT_Display_CLK_Num(CS5_PIN, 3, TFTConfig.theme_index);
		// TFT_Display_Image(CS2_PIN,CS5_PIN, pic_systemTheme_SetPIC);
		TFT_Display_Image_Select(CS2_PIN, pic_systemTheme_Pic);
